...
import { classroomService, quizService } from '../../services/dataService';
import StatsCard from '../../components/StatsCard';
import {
  Box, Grid, Typography, Card, CardContent, Button, Chip, TextField,
  Dialog, DialogTitle, DialogContent, DialogActions
} from '@mui/material';
import { School, Quiz, MenuBook, Add, Translate, VolumeUp } from '@mui/icons-material';
...
const StudentDashboard = () => {
  ...
  const loadData = async () => {
    try {
      ...
      setClassrooms([
        { id: 1, name: 'Science - Grade 8', subject: 'Science', grade: '8', teacherName: 'Demo Teacher', lessonCount: 12, quizCount: 5 },
        { id: 2, name: 'Hindi Literature', subject: 'Hindi', grade: '8', teacherName: 'Demo Teacher', lessonCount: 6, quizCount: 2 },
      ]);
    }
    setLoading(false);
  };
  ...
  return (
    <Box sx={{ animation: 'fadeIn 0.4s ease' }}>
      ...
      <Grid container spacing={3} sx={{ mb: 4 }}>
        ...
        <Grid item xs={12} sm={6} md={3}>
          <StatsCard title="Upcoming Quizzes" value={3} icon={<Quiz sx={{ color: '#F59E0B' }} />} color="#F59E0B" delay={0.1} />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <StatsCard title="Lessons Available" value={18} icon={<MenuBook sx={{ color: '#10B981' }} />} color="#10B981" delay={0.2} />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <StatsCard title="Translations Used" value={24} icon={<Translate sx={{ color: '#8B5CF6' }} />} color="#8B5CF6" delay={0.3} />
        </Grid>
      </Grid>

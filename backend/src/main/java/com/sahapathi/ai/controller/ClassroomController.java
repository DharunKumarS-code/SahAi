...
public class ClassroomController {
    ...
    public ResponseEntity<ApiResponse<ClassroomResponse>> create(
        ...
        ClassroomResponse response = classroomService.createClassroom(request, user.getId());
        return ResponseEntity.ok(ApiResponse.success("Classroom created", response));
    }

    @GetMapping("/{id}")
    @Operation(summary = "Get classroom by ID")
    public ResponseEntity<ApiResponse<ClassroomResponse>> getById(@PathVariable String id) {
    ...
    public ResponseEntity<ApiResponse<String>> join(
            ...
            @AuthenticationPrincipal UserDetails userDetails) {
        User user = getUser(userDetails);
        String result = classroomService.joinClassroom(inviteCode, user.getId());
